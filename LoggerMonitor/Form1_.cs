...
namespace LoggerMonitor
{
	public partial class Form1 : Form
    {
        ...
        {
            OnOff1_label.Text = raceFile.OnOffName[0];
            OnOff2_label.Text = raceFile.OnOffName[1];
            OnOff3_label.Text = raceFile.OnOffName[2];
            OnOff4_label.Text = raceFile.OnOffName[3];
        }
        ...
        private void UpdateOnOffs(raceFileClass raceFile)
        {
            ...
            if (raceFile.onOff[3] == true)
                if (sets.OnOffs.getState(3) == true)
                    OnOff4_progressBar.Value = 100;
                else
                    OnOff4_progressBar.Value = 0;
            else
                if (sets.OnOffs.getState(3) == true)
                    OnOff4_progressBar.Value = 0;
                else
        ...
        private void UpdateLoggerGauges(raceFileClass raceFile)
        {
            ...
            aGauge1.CapText = vText;
            gauge1_label.Text = aGauge1.Value.ToString();

            aGauge2.Value = raceFile.RPM[1];
            name = System.Text.Encoding.ASCII.GetBytes(raceFile.RPMLimit[1].name);
            vText = System.Text.Encoding.ASCII.GetString(name);
            aGauge2.CapText = vText;
            gauge2_label.Text = aGauge2.Value.ToString();
            ...
            aGauge5.Value = (float)raceFile.AnalogDouble[0];
            aGauge6.Value = (float)raceFile.AnalogDouble[1];
            aGauge7.Value = (float)raceFile.AnalogDouble[2];
            aGauge8.Value = (float)raceFile.AnalogDouble[3];
            aGauge9.Value = (float)raceFile.AnalogDouble[4];
            aGauge10.Value = (float)raceFile.AnalogDouble[5];
            aGauge11.Value = (float)raceFile.AnalogDouble[6];
            aGauge12.Value = (float)raceFile.AnalogDouble[7];

            aGauge5.CapText = formLoggerCapText(0);
            aGauge6.CapText = formLoggerCapText(1);
            aGauge7.CapText = formLoggerCapText(2);
            aGauge8.CapText = formLoggerCapText(3);
            aGauge9.CapText = formLoggerCapText(4);
            aGauge10.CapText = formLoggerCapText(5);
            aGauge11.CapText = formLoggerCapText(6);
            aGauge12.CapText = formLoggerCapText(7);
        ...
        private void UpdateGaugeSettings(string settings)
        {
            ...
            if (items[0].Contains("HeaderItems")) 
            {
                if(items.Count() >= (1 + 4 + 4 + 16))
                {
                    ...
                    for (i = 0; i < 4; i++)
                    {
                        monFile.RPMLimit[i].name = items[i + 1].ToCharArray();
                    }
                    for (i = 0; i < 4; i++)
                    {
                    ...
                    for (i = 0; i < 16; i++)
                    {
                        monFile.AnalogLimit[i].name = items[i + 9].ToCharArray();
                    }
                    //updateGauges(monFile);
        ...
        private void UpdateGaugeLimits(string limits)
        {
            ...
            if (items[0].Contains("HeaderValues") && items.Count() >= (1 + 8 + 32))
            {
                ...
                for (i = 0; i < 16; i++)
                {
                    ...
                    {
                        case 0:
                            aGauge5.MaxValue = (float)max;
                            aGauge5.MinValue = (float)min;
                            aGauge5.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 01:
                            aGauge6.MaxValue = (float)max;
                            aGauge6.MinValue = (float)min;
                            aGauge6.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 02:
                            aGauge7.MaxValue = (float)max;
                            aGauge7.MinValue = (float)min;
                            aGauge7.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 03:
                            aGauge8.MaxValue = (float)max;
                            aGauge8.MinValue = (float)min;
                            aGauge8.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 04:
                            aGauge9.MaxValue = (float)max;
                            aGauge9.MinValue = (float)min;
                            aGauge9.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 05:
                            aGauge10.MaxValue = (float)max;
                            aGauge10.MinValue = (float)min;
                            aGauge10.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 06:
                            aGauge11.MaxValue = (float)max;
                            aGauge11.MinValue = (float)min;
                            aGauge11.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 07:
                            aGauge12.MaxValue = (float)max;
                            aGauge12.MinValue = (float)min;
                            aGauge12.ScaleLinesMajorStepValue = (float)majorStep;
                            break;
                        case 08:
